...
C.am = 1.66053892e-27; % atomic mass unit in kg
...
Pty0in = AtomType == 0;
Pty1in = AtomType == 1;
Pty2in = AtomType == 2;%added

nAtoms0 = sum(Pty0in);
nAtoms1 = sum(Pty1in);
nAtoms2 = sum(Pty2in);%added
...
while t < TStop
    ...
    elseif Method == 'VE'
        ...
        x(Pty2in) = -xpp(Pty2in) + 2 * xp(Pty2in) + dt^2 / Mass2 * Fx(Pty2in);%added

        y(Pty0in) = -ypp(Pty0in) + 2 * yp(Pty0in) + dt^2 / Mass0 * Fy(Pty0in);
        y(Pty1in) = -ypp(Pty1in) + 2 * yp(Pty1in) + dt^2 / Mass1 * Fy(Pty1in);
        y(Pty2in) = -ypp(Pty2in) + 2 * yp(Pty2in) + dt^2 / Mass2 * Fy(Pty2in);%added
    ...
    if nAtoms1
        V2_1 = (Vx(Pty1in).*Vx(Pty1in)+Vy(Pty1in).*Vy(Pty1in));
    elseif nAtoms2
        V2_2 = (Vx(Pty2in).*Vx(Pty2in)+Vy(Pty2in).*Vy(Pty2in));%added
    else
        V2_1 = 0;
        V2_2 = 0;%added
    end
    ...
    KETot(c) = (KE0 * nAtoms0 + KE1 * nAtoms1 + KE2 * nAtoms2);%added
    T(c) = KETot(c) / nAtoms / C.kb;
    T0(c) = KE0 / C.kb;
    T1(c) = KE1 / C.kb;
    T2(c) = KE2 / C.kb; %added
